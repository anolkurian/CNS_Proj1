...
def gradient_of_loss_wrt_input(model, x, y):
    ...
    with tf.GradientTape() as tape:
        tape.watch(x)  # Watch the input tensor 'x' to compute its gradient
        y_pred = model(x)  # Forward pass to obtain the predicted class probabilities
        loss = tf.keras.losses.CategoricalCrossentropy()(y, y_pred)  # Compute the loss
