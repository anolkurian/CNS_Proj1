...
def distortion(x_in, x_adv, p=2):
    ## TODO ##
    ## Insert your code here to calculate the distortion
    diff = x_adv - x_in
    dist = tf.norm(tf.reshape(diff, [-1]), ord=p)
...
def random_images(size=(1,28*28)):
    ...
    images = np.random.randint(0, 256, size=(num_images, image_size))
    return images
...
def randomized_smoothing_predict_fn(model, x, sigma=1.0, num_samples=100, noise_type='Gaussian'):
    ...
    for i in range(0, num_samples):
        ...
        elif noise_type == 'Laplace' or noise_type == 'laplace':
            ## 1. Generate Laplace noise with mean 0 and scale sigma/√2
            noise = np.random.laplace(loc=0.0, scale=sigma / np.sqrt(2), size=x.shape)
        else:
            raise ValueError('Invalid noise type: {}'.format(noise_type))
        
        ## 2. Add the noise to the input data
        x_noisy = x + noise
...
def main():
    ...
    ########################################################
        ...
        plt.plot(sigmas, adv_accs, label='Adversarial')
        plt.plot(sigmas, benign_accs, label='Benign')
        plt.xlabel('Sigma')
        plt.ylabel('Accuracy')
        plt.title('FGSM Attack Evaluation')
        plt.legend()
        plt.show()
        # raise NotImplementedError()
